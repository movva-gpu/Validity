    ...
    .addSubcommand((subcommand) => {
        for (const locale in langs) {
            subcommand.setDescriptionLocalization(
                ...
                langs[locale].commands.system.subcommands?.help
        ...
        return subcommand
            .setName('help')
            .setDescription(
                langs['en-US'].commands.system.subcommands?.help
                    .description as string
            );
    })
...
for (const locale in langs) {
    data.setDescriptionLocalization(
        locale as LocaleString,
        langs[locale].commands.system.description as string
    );
}
...
export async function execute(interaction: ChatInputCommandInteraction) {
    ...
    if (subCommand != '') {
        ...
        switch (subCommand) {
            case 'help':
                invokeHelpEmbed(
                    InitialHelpEmbedButton.SystemSubCommands,
                    interaction
                );
                break;
            case 'create':
...
function handleInteractionReply(
    result: unknown,
    interaction: ChatInputCommandInteraction,
    commandName = interaction.options.getSubcommand() as string
) {
    let responses =
        (langs[interaction.locale] &&
            langs[interaction.locale].commands &&
            langs[interaction.locale].commands.system &&
            langs[interaction.locale].commands.system.responses) ??
        (langs['en-US'] &&
            langs['en-US'].commands &&
            langs['en-US'].commands.system &&
    ...
    let success =
        (langs[interaction.locale] &&
            langs[interaction.locale].commands &&
            langs[interaction.locale].commands.system &&
            langs[interaction.locale].commands.system.subcommands &&
            langs[interaction.locale].commands.system.subcommands?.[
                commandName
            ] &&
            langs[interaction.locale].commands.system.subcommands?.[commandName]
                .success) ??
        (langs['en-US'] &&
            langs['en-US'].commands &&
            langs['en-US'].commands.system &&
            langs['en-US'].commands.system.subcommands &&
            langs['en-US'].commands.system.subcommands[commandName] &&
    ...
    if (typeof result === 'number') {
        if (!responses) {
            interaction.reply('Response missing in files');
            return;
        }
        if (
            !responses[
            ...
            interaction.reply(
                'Response missing in files for ' + DatabaseError[result] ??
                    OptionError[result] ??
                    SystemError[result]
            );
            return;
    ...
    if (result instanceof MessageCollector) {
        interaction.reply(
            specific ??
                'Specific result missing in files for `/system ' +
                    commandName +
                    '`.'
        );
        result.on('collect', async (message) => {
            ...
            message.channel.send(
                success ??
                    'Success result missing in files for `/system ' +
                        commandName +
                        '`.'
            );
        });
