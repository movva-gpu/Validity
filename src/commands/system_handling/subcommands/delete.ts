...
export const data = new SlashCommandSubcommandBuilder()
    .setName('delete')
    .setDescription(
        langs['en-US'].commands.system.subcommands?.delete.description as string
    );
...
): Promise<number | void | MessageCollector> {
    if (
        !userHasSystem(
            interaction.user,
            readDatabase() ?? require('../../../../data/data.json')
        )
    )
        return SystemError.NoUserSys;
