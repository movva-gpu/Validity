...
import {
    ...
    OptionError,
    SystemsDataType,
} from './commands/system_handling/system.ts';
...
export function createFullEmbed(
    title: string,
    description: string | null = null,
    thumbnail: string | null = null,
    image: string | null = null,
    url: string | null = null,
    footer = embedDefaults.footer,
    color = embedDefaults.color as ColorResolvable,
    timestamp = true
): EmbedBuilder {
...
export function invokeHelpEmbed(
    initialMainButton: InitialHelpEmbedButton,
    interaction: ChatInputCommandInteraction
): void {
    let helpTexts = langs['en-US'].helpTexts as any;

    if (langs[interaction.locale]?.helpTexts)
        helpTexts = langs[interaction.locale].helpTexts;
    const informationEmbed = createFullEmbed(
        helpTexts.info.title,
        helpTexts.info.description,
        embedDefaults.image
    ...
    const systemSubCommandsEmbed = createEmbed(
        ...
        helpTexts.systemSubcommands.fields[0],
        helpTexts.systemSubcommands.fields[1],
        helpTexts.systemSubcommands.fields[2]
    );

    const informationButton = createButton(
        '\u2139',
        ButtonStyle.Secondary,
        helpTexts.info.buttonLabel
    ).setCustomId('info');
    const systemSubCommandsButton = createButton(
        '📜',
        ButtonStyle.Secondary,
        helpTexts.systemSubcommands.buttonLabel
...
export function createButton(
    ...
    return new ButtonBuilder().setEmoji(emoji).setStyle(style).setLabel(label);
}

export function stringOptionNormalize(
    interaction: ChatInputCommandInteraction,
    optionName: string,
    required: true
): string;
...
export function stringOptionNormalize(
    ...
    if (interaction.options.getString(optionName, required) == null)
        return undefined;

    return interaction.options.getString(optionName, required) as string;
}

export function saveDatabase<T, K>(
    newDatabase: SystemsDataType,
    objectToReturnOnError = DatabaseError.SavingError as T,
    objectToReturnOnSuccess?: K
): T | K | void {
    fs.writeFile('data/data.json', JSON.stringify(newDatabase), function (err) {
        if (err) {
            console.error(err);
            return objectToReturnOnError;
        }
...
export function userHasSystem(
    user: User,
    systemsData: SystemsDataType
): boolean {
    let result = false;
...
export function getUserSystemIndex(
    user: User,
    systemsData: SystemsDataType
): number | void {
...
export async function getUrlResponse<T>(
    url: URL | string,
    toReturnOn404 = OptionError.ImageUrl404 as T,
    toReturnOnBroken = OptionError.ImageUrlIsBroken as T,
    toReturnOnWrongType = OptionError.ImageWrongType as T,
    toReturnOnSuccess: any = false
): Promise<T | false> {
    ...
    await fetch(url)
        .then((response) => {
            response.headers.forEach((element, key) => {
                if (key === 'content-type' && !element.startsWith('image'))
                    toReturn = toReturnOnWrongType;
                return;
            });
